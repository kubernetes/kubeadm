...
func kubeadmUpgradePlan(c *status.Cluster, cp1 *status.Node, configVersion string, upgradeVersion *version.Version, ignorePreflightErrors string, vLevel int) error {
	planArgs := []string{
