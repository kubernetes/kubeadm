...
// NOTE: this is fork of the validators in the kind internal configuration
// adapted to be able to validate the kind public type.

// ValidateCluster returns a ConfigErrors with an entry for each problem
// with the config, or nil if there are none
func ValidateCluster(c *v1alpha3.Cluster) error {
	errs := []error{}
	...
	// validate nodes
	numByRole := make(map[v1alpha3.NodeRole]int32)
	...
	numControlPlane, anyControlPlane := numByRole[v1alpha3.ControlPlaneRole]
	if !anyControlPlane || numControlPlane < 1 {
		errs = append(errs, errors.Errorf("must have at least one %s node", string(v1alpha3.ControlPlaneRole)))
	}
...
// ValidateNode returns a ConfigErrors with an entry for each problem
// with the Node, or nil if there are none
func ValidateNode(n *v1alpha3.Node) error {
	errs := []error{}

	// validate node role should be one of the expected values
	switch n.Role {
	case v1alpha3.ControlPlaneRole,
		v1alpha3.WorkerRole:
	default:
